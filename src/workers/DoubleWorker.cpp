    ...
    while (Workers::sequence() > 0) {
        ...
        while (!Workers::isOutdated(m_sequence)) {
            ...
            *Job::nonce(m_state->blob + m_state->job.size()) = ++m_state->nonce2;
            ...
            uint32_t diffResult1 = (m_hash[0] << 8) + m_hash[1];
            if (diffResult1 < m_state->job.target()) {
                Workers::submit(JobResult(m_state->job.poolId(), m_state->job.id(), m_state->nonce1, m_hash, m_state->job.diff()));
            }

            uint32_t diffResult2 = (m_hash[LEN::RESULT] << 8) + m_hash[LEN::RESULT + 1];
            if (diffResult2 < m_state->job.target()) {
                Workers::submit(JobResult(m_state->job.poolId(), m_state->job.id(), m_state->nonce2, m_hash + LEN::RESULT, m_state->job.diff()));
            }
...
void DoubleWorker::consumeJob()
{
    ...
    if (m_state->job.isNicehash()) {        
        ...
        m_state->nonce1 = m_state->job.jobId() + (m_state->job.jobUnit() / (m_threads * 2) * m_id);
        m_state->nonce2 = m_state->job.jobId() + (m_state->job.jobUnit() / (m_threads * 2) * (m_id + m_threads));
    }
}
